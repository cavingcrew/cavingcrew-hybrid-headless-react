...
final class Hybrid_Headless_Addon extends Addon {
	...
	public function register_variables( $variables ) {
        // Register variables by providing the absolute file path.
        // AutomateWoo's registry will include the file when the variable is needed.
        // The variable file MUST end with 'return new ClassName();'
        $variable_path = $this->path( '/includes/variables/' );

		$variables['product']['event_start_date'] = $variable_path . 'product-event-start-date.php';
		$variables['product']['event_finish_date'] = $variable_path . 'product-event-finish-date.php';
        $variables['product']['event_data'] = $variable_path . 'product-event-data-variable.php';
