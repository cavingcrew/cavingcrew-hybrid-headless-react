...
export type ChallengeDomain = 'claustrophobia' | 'water' | 'heights' | 'hazard' | 'endurance';
 ...
 */
function calculateClaustrophobiaScore(difficulty: any): { score: number, details: ChallengeMetric['details'] } {
  const psychological = parseDifficultyValue(difficulty.route_difficulty_psychological_claustrophobia);
  const objective = parseDifficultyValue(difficulty.route_difficulty_objective_tightness);
  ...
  const details = [
    {
      ...
      contribution: psychological !== null ? psychological * CHALLENGE_CONFIG.weights.claustrophobia.psychological : 0
    },
    {
      key: 'objective',
      ...
      contribution: objective !== null ? objective * CHALLENGE_CONFIG.weights.claustrophobia.objective : 0
    }
  ];
 ...
 */
function calculateWaterScore(difficulty: any): { score: number, details: ChallengeMetric['details'] } {
  const wetness = parseDifficultyValue(difficulty.route_difficulty_wetness);
  const nearFace = parseDifficultyValue(difficulty.route_difficulty_water_near_face);
  const deepWater = parseDifficultyValue(difficulty.route_difficulty_exposure_to_deep_water);
  ...
  const details = [
    {
      ...
      contribution: wetness !== null ? wetness * CHALLENGE_CONFIG.weights.water.wetness : 0
    },
    {
      key: 'nearFace',
      ...
      contribution: nearFace !== null ? nearFace * CHALLENGE_CONFIG.weights.water.nearFace : 0
    },
    {
      key: 'deepWater',
      ...
      contribution: deepWater !== null ? deepWater * CHALLENGE_CONFIG.weights.water.deepWater : 0
    },
    {
      key: 'muddiness',
      label: 'Muddiness',
      ...
      contribution: muddiness !== null ? muddiness * CHALLENGE_CONFIG.weights.water.muddiness : 0
    }
  ];
 ...
 */
function calculateHeightsScore(difficulty: any): { score: number, details: ChallengeMetric['details'] } {
  const exposure = parseDifficultyValue(difficulty.route_difficulty_exposure_to_heights);
  const climbing = parseDifficultyValue(difficulty.route_difficulty_technical_climbing_difficulty);
  ...
  const details = [
    {
      ...
      contribution: exposure !== null ? exposure * CHALLENGE_CONFIG.weights.heights.exposure : 0
    },
    {
      key: 'climbing',
      ...
      contribution: climbing !== null ? climbing * CHALLENGE_CONFIG.weights.heights.climbing : 0
    }
  ];
 ...
 */
function calculateHazardScore(difficulty: any): { score: number, details: ChallengeMetric['details'] } {
  const hazard = parseDifficultyValue(difficulty.route_difficulty_objective_hazard);
 ...
 */
function calculateEnduranceScore(difficulty: any): { score: number, details: ChallengeMetric['details'] } {
 ...
 */
export function extractChallengeMetrics(routeData?: Route["acf"]): ChallengeMetric[] | null {
