...
import { determineSignupStatus, getStatusColor } from "../../utils/trip-participant-utils";
...
export function NeoClanVolunteeringRoles({ trip, onRoleAssigned }: NeoClanVolunteeringRolesProps) {
  ...
  const canAssignRoles = Auth.isAdmin(user, accessLevel) || 
  ...
  if (participants.length === 0) {
    return (
      <Paper withBorder p="md" radius="md">
        <Title order={3} mb="md">Volunteer Roles</Title>
  ...
  return (
    <Paper withBorder p="md" radius="md">
              ...
              <Table.Tr key={participant.order_id}>
                ...
                <Table.Td>
                  ...
                  participant.order_meta.cc_volunteer !== "none" ? (
                    <Badge color="green" leftSection={<IconHeartHandshake size={14} />}>
                      {participant.order_meta.cc_volunteer}
                    </Badge>
                  ) : (
      ...
      <Modal opened={opened} onClose={close} title="Assign Volunteer Role" centered>
        {selectedParticipant && (
          <Stack>
            <Text>
              Assigning role to {selectedParticipant.first_name} {selectedParticipant.last_name}
            </Text>
