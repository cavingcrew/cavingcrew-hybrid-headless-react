...
const requiresSRT = (trip: Trip) => {
	...
	const isSRTTraining = trip.categories.some(
		(cat) => cat.slug === "srt-training",
	);

	// Check gear requirements from ACF
	const gearRequiresSRT = trip.acf.event_gear_required
		?.toLowerCase()
		.includes("srt");

	// Check skills requirements from ACF
	const skillsRequiresSRT = trip.acf.event_skills_required
		?.toLowerCase()
		.includes("srt");

	// Check route description for SRT mentions
	const routeHasSRT =
		trip.route?.acf?.route_blurb?.toLowerCase().includes("srt") ||
