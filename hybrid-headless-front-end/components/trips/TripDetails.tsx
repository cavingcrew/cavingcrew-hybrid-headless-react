...
export function TripDetails({ trip }: TripDetailsProps) {
	...
	const getVagueLocation = (address?: string) => {
		...
		return (
			localityCandidates.find(
				(part) => part.length > 3 && part.match(/[a-z]/i),
			) || null
		);
	};
	...
	const startDate = acf?.event_start_date_time
		...
		: null;
	const endDate = acf?.event_finish_date_time
		? new Date(acf.event_finish_date_time)
		: null;
	const isOvernightTrip = trip.categories.some(
		(cat) => cat.slug === "overnight-trips",
	);
	...
	return (
		<Stack gap="xl">
				...
				<Grid.Col span={{ base: 12, md: 8 }}>
						...
						<Stack gap="md">
								...
								<Group gap="xs" wrap="nowrap" align="flex-start">
									...
									<Text>
										{hasPurchased && trip.hut?.hut_lat_long ? (
											...
											`Region: ${
												trip.hut?.hut_location?.post_title ||
							...
							{acf?.event_trip_leader && (
								<Group gap="xs" wrap="nowrap" align="flex-start">
									...
									<Text>Lead by: {acf.event_trip_leader}</Text>
								</Group>
							)}
							<Group gap="xs" wrap="nowrap" align="flex-start">
							...
							{acf?.event_non_members_welcome !== "no" &&
								...
								String(acf?.event_cost || "").replace(/\s+/g, "") !==
									String(trip.price).replace(/\s+/g, "") && (
									<Group gap="xs">
										<IconCoin size={20} />
					...
					{(acf?.event_skills_required ||
						...
						(acf?.event_volunteering_required &&
							acf.event_volunteering_required > 0) ||
						(acf?.event_attendance_required &&
							acf.event_attendance_required > 0)) && (
									...
									acf.event_volunteering_required > 0 && (
										<Group gap="xs">
											...
											<div>
												...
												<Text>
													Contribute to {acf.event_volunteering_required} events
												</Text>
											</div>
										</Group>
									)}
									...
									<Group gap="xs">
										...
										<div>
											...
											{acf.event_u18s_come === "yes" && (
												...
												>
													This trip is specifically for families with under-18s.
													Adults without accompanying minors cannot participate.
												</Alert>
											)}
										</div>
									</Group>
			...
			{hasPurchased ? (
				<>
					<Paper withBorder p="md" radius="md" id="trip-signup-section">
						...
						>
							<Text>
								You're already booked on this trip. Check your email for
								confirmation or visit your{" "}
			...
			{showSignupCTAs && (
				<Center mt="xl">
					<Button size="lg" color="blue" onClick={scrollToSignup}>
			...
			{hasPurchased && isOvernightTrip && (
				<TripParticipantInfo hut={trip.hut} tripId={trip.id} />
			)}
