...
import { Modal, Title, Alert, Text, Group, Button, Stack, Box } from '@mantine/core';
...
export function EmergencyInfoModal({ opened, onClose, participant }: EmergencyInfoModalProps) {
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      ...
      {participant && (
        <Stack>
          ...
          <Group>
            ...
            <Text>{participant.admin_meta?.['admin-phone-number'] || participant.admin_meta?.['billing_phone'] || 'Not provided'}</Text>
          </Group>

          <Group align="flex-start">
            <Text fw={700}>Emergency Contact:</Text>
            <Box>
              <Text>{participant.admin_meta?.['admin-emergency-contact-name'] || 'Not provided'}</Text>
              <Text>{participant.admin_meta?.['admin-emergency-contact-phone'] || 'No phone provided'}</Text>
              <Text>{participant.admin_meta?.['admin-emergency-contact-relationship'] || 'Relationship not specified'}</Text>
            </Box>
          </Group>
          ...
          <Group align="flex-start">
            ...
            <Box>
              <Text>{participant.admin_meta?.['billing_address_1'] || 'Not provided'}</Text>
          ...
          <Group>
            ...
            <Text>{participant.admin_meta?.['admin-date-of-birth'] || 'Not provided'}</Text>
          </Group>

          <Group>
            <Text fw={700}>Car Registration:</Text>
            <Text>{participant.admin_meta?.['admin-car-registration'] || 'Not provided'}</Text>
          </Group>

          <Group align="flex-start">
            <Text fw={700}>Health Information:</Text>
            <Box>
              <Text>{participant.admin_meta?.['admin-diet-allergies-health-extra-info'] || 'None provided'}</Text>
