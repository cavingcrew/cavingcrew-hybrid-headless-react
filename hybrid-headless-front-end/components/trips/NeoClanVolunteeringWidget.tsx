...
const MarkAttendanceModal = ({
	...
	const steps = [
		...
		},
		{
			...
			content: (
				<Radio.Group value={status} onChange={setStatus} name="lateBailDetails">
					<Stack>
	...
	return (
		<Modal opened={true} onClose={onClose} title={steps[step].title}>
			<Stack>
				{steps[step].content}
	...
	const handleAttendanceConfirm = async (status: string) => {
		...
		try {
			const response = await apiService.updateAttendanceStatus(
				attendanceParticipant.order_id,
				status,
			);
			...
			if (response.success) {
				notifications.show({
					...
					message: `${attendanceParticipant.first_name}'s status set to ${status}`,
					color: "green",
				});
				refetch();
			}
